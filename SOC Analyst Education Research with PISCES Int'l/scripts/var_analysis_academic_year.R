...
data_recast <- data_rescale %>%
  mutate(time_to_resolve = factor(ifelse(time_to_resolve=="Week" | time_to_resolve=="Quarter" | time_to_resolve=="School year", "Within academic year", "More than academic year")))
factor <- data.frame(data_rescale$time_to_resolve)
ncol(factor)
graph <- lapply(names(factor),
                function(x) 
...
ggplot(data_recast, aes(x = length_desc, y = bug_actions)) +
  geom_point(aes(color = time_to_resolve),
             size = 0.5) +
  stat_smooth(method = 'lm',
              formula = y~poly(x, 2),
...
#POTENTIALLY GOOD
nl1 = ggplot(data_recast, aes(x = length_steps_to_repro, y = bug_actions)) +
  geom_point(aes(color = time_to_resolve),
             size = 0.5) +
  stat_smooth(method = 'lm',
              formula = y~poly(x, 2),
...
#POTENTIALLY GOOD
ggplot(data_recast, aes(x = length_additional_info, y = bug_actions)) +
  geom_point(aes(color = time_to_resolve),
             size = 0.5) +
  stat_smooth(method = 'lm',
              formula = y~poly(x, 2),
